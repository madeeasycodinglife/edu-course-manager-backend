...
public class ValidationUtils {
    ...
    // Define the pattern for the course code
    private static final Pattern COURSE_CODE_PATTERN = Pattern.compile("^[A-Z]{2} \\d+$");
    ...
    public static Map<String, String> validateCourseCode(String courseCode) {
        Map<String, String> errors = new HashMap<>();
        if (!COURSE_CODE_PATTERN.matcher(courseCode).matches()) {
            errors.put("courseCode", "Course code must start with 'CS', followed by a space and a number");
        }
        return errors;
    }
}
